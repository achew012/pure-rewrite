...
class spanNER(pl.LightningModule):
    ...
    def _get_span_embeddings(self, longformer_outputs, spans, span_mask):
        ...
        cls_embeddings = longformer_outputs[0][:, :1].squeeze()[
    ...
    def forward(self, **batch):
        ...
        if labels is not None:
            ...
            if self.entity_loss_weights is not None:
                loss_fct.weight = self.entity_loss_weights.to(self.device)
    ...
    def test_epoch_end(self, outputs):
        ...
        reconstructed_preds = [[(span[0], span[1], entity_labels[span[2]]) for span in sample.cpu().detach().tolist() if span[2] is not 0]
